...
class AccountInvoiceReport(models.Model):
    ...
    def _table_query(self):
        query = '''SELECT MIN(a.id) as id, a.move_id AS move_id, MIN(a.product_id) AS product_id, MIN(a.account_id) account_id, MIN(a.analytic_account_id) analytic_account_id, 
		        MIN(a.journal_id) journal_id, MIN(a.company_id) company_id, MIN(a.company_currency_id) company_currency_id,  MIN(a.commercial_partner_id) commercial_partner_id, 
		        MIN(a.state) state, MIN(a.move_type) move_type, MIN(a.partner_id) partner_id, MIN(a.invoice_user_id) invoice_user_id, MIN(a.fiscal_position_id) fiscal_position_id, 
		        MIN(a.payment_state) payment_state, MIN(a.invoice_date) invoice_date, MIN(a.invoice_date_due) invoice_date_due, MIN(a.product_uom_id) product_uom_id, 
		        MIN(a.product_categ_id) product_categ_id, MIN(a.quantity) quantity, MIN(a.price_subtotal) price_subtotal, MIN(a.price_average) price_average, MIN(a.country_id) country_id
            FROM (''' + \
                '%s %s %s' % (self._select(), self._from(), self._where()) + \
            ''' ) as a
                GROUP BY move_id
                '''
        return query
